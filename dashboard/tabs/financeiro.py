            ...
            if not df_tmp.empty:
                ...
                with col_evol:
                    st.html(
                        ...
                        '<div style="display:flex;flex-wrap:wrap;align-items:center;justify-content:space-between;gap:0.75rem;margin-bottom:0.25rem">'
                        '<div style="display:flex;flex-direction:column;gap:2px">'
                        '<span style="color:#94A3B8;font-size:0.85rem;font-weight:500">'
                        '<span class="material-symbols-rounded" style="font-size:1rem;vertical-align:middle">trending_up</span> Evolução Financeira</span>'
                        '<div style="display:flex;flex-wrap:wrap;align-items:baseline;gap:10px">'
                        f'<span style="color:#F8FAFC;font-size:1.5rem;font-weight:700;letter-spacing:-0.02em;line-height:1.3">R$ {income_total:,.2f}</span>'
                        f'<span style="background:#166534;color:#86efac;border-radius:100px;padding:2px 10px;font-size:0.75rem;font-weight:600;display:inline-flex;align-items:center;gap:3px;white-space:nowrap;flex-shrink:0">▲ {net_margem:.1f}%</span>'
                        '</div></div></div>'
                    )
                    st.markdown(
                    ...
                    leg_col, per_col = st.columns([4.5, 1], gap="small", vertical_alignment="center")
                    with leg_col:
                        st.html(
                            '<div style="display:flex;gap:16px;align-items:center;font-size:0.85rem;color:#94A3B8">'
                            '<span style="display:flex;align-items:center;gap:6px">'
                            '<span style="width:10px;height:10px;border-radius:50%;background:#818CF8"></span>Receitas</span>'
                            '<span style="display:flex;align-items:center;gap:6px">'
                            '<span style="width:10px;height:10px;border-radius:50%;background:#F87171"></span>Despesas</span>'
                            '</div>'
                        )
                    with per_col:
                ...
                with col_side:
                    ...
                    with sub_lucro:
                        ...
                        st.html(
                            ...
                            '<span class="material-symbols-rounded" style="font-size:1rem;vertical-align:middle">bar_chart</span> Lucro Líquido</div>'
                            '<div style="display:flex;flex-wrap:wrap;align-items:baseline;gap:10px">'
                            f'<span style="color:#F8FAFC;font-size:1.25rem;font-weight:700;letter-spacing:-0.02em;line-height:1.3">R$ {total_net:,.2f}</span>'
                            f'<span style="background:{"#166534" if net_pos else "#7f1d1d"};color:{"#86efac" if net_pos else "#fca5a5"};border-radius:100px;padding:2px 10px;font-size:0.75rem;font-weight:600;display:inline-flex;align-items:center;gap:3px;white-space:nowrap;flex-shrink:0">'
                            f'{"▲" if net_pos else "▼"} {net_pct:.1f}%</span>'
                            '</div></div>'
                    ...
                    st.html(
                        '<div style="width:100%;height:0.1rem;background:#334155;margin:0 auto"></div>'
                    )
                    ...
                    with sub_cat:
                        ...
                        if not expense_df.empty:
                            ...
                            st.html(
                                ...
                                '<div>'
                                '<div style="color:#94A3B8;font-size:0.85rem;font-weight:500">'
                                '<span class="material-symbols-rounded" style="font-size:1rem;vertical-align:middle">receipt_long</span> Despesas por Categoria</div>'
                                '<div style="display:flex;flex-wrap:wrap;align-items:baseline;gap:10px">'
                                f'<span style="color:#F8FAFC;font-size:1.25rem;font-weight:700;letter-spacing:-0.02em;line-height:1.3">R$ {total_exp:,.2f}</span>'
                                '</div></div>'
                                f'<span style="color:#94A3B8;font-size:0.75rem">{len(cat_expense)} categorias</span>'
                                '</div></div></div>'
                            )
                            fig_cat = px.pie(
                ...
                if sel_points:
                    ...
                    if not detail.empty:
                        st.markdown("---")
                        st.markdown(f'<span style="color:#94A3B8;font-size:0.85rem;font-weight:500"><span class="material-symbols-rounded" style="font-size:1rem;vertical-align:middle">content_paste</span> Detalhamento: {periodo_clicado}</span>', unsafe_allow_html=True)
